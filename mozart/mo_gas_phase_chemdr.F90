...
!
    ...
    hg2_ndx = get_spc_ndx('Hg2')
    call cnst_get_ind( 'Hg2', rgm_id, abort=.false. )
...
!-----------------------------------------------------------------------
    ...
    call addfld( 'hg2tend2', (/ 'lev' /), 'A', 'kg/kg/s', 'Hg2 tend before RGM sea-salt uptake' )
    call addfld( 'RGM_mmr2', (/ 'lev' /), 'A', 'kg/kg', 'RGM mmr in sea-salt uptake' )
...
!
    ...
    use mo_rgm_seasalt_uptake,  only : rgm_seasalt_uptake
...
!            ratecon_sfstrat needs total hcl.
    ...
    call usrrxt( reaction_rates, tfld, ion_temp_fld, ele_temp_fld, invariants, h2ovmr, &
                 ...
                 troplevchem, dlats, ncol, sad_trop, reff, cwat, mbar, pbuf, lchnk, cldfr, vmr, zi )
...
!
    ...
    wind_spd(:ncol) = sqrt( ufld(:ncol,pver)*ufld(:ncol,pver) + vfld(:ncol,pver)*vfld(:ncol,pver) )
    call outfld( 'hg2tend2', qtend(:ncol,:,rgm_id), ncol, lchnk )
    call outfld( 'RGM_mmr2', mmr(:ncol,:,hg2_ndx), ncol, lchnk )
    call rgm_seasalt_uptake( lchnk, ncol, ocnfrac(:ncol), relhum, wind_spd, pdel(:ncol,:), delt_inverse, mmr(:ncol,:,hg2_ndx), qtend(:ncol,:,rgm_id))
