module mo_photo
  ...
  integer :: jo1d_ndx, jo3p_ndx, jno2_ndx, jn2o5_ndx, jhg2_ndx, jhgp_ndx
  ...
  integer :: pom1_ndx, pom4_ndx, soa11_ndx, soa12_ndx, soa21_ndx, soa22_ndx 
  integer :: soa31_ndx, soa32_ndx, soa41_ndx, soa42_ndx, soa51_ndx, soa52_ndx 
...
contains
  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    jhg2_ndx = get_rxt_ndx( 'jhg2' )
    jhgp_ndx = get_rxt_ndx( 'jhgp' )
    ...
    pom1_ndx  = get_spc_ndx('pom_a1')
    pom4_ndx  = get_spc_ndx('pom_a4')
    soa11_ndx = get_spc_ndx('soa1_a1')
    soa12_ndx = get_spc_ndx('soa1_a2')
    soa21_ndx = get_spc_ndx('soa2_a1')
    soa22_ndx = get_spc_ndx('soa2_a2')
    soa31_ndx = get_spc_ndx('soa3_a1')
    soa32_ndx = get_spc_ndx('soa3_a2')
    soa41_ndx = get_spc_ndx('soa4_a1')
    soa42_ndx = get_spc_ndx('soa4_a2')
    soa51_ndx = get_spc_ndx('soa5_a1')
    soa52_ndx = get_spc_ndx('soa5_a2')
  ...
  subroutine table_photo( photos, pmid, pdel, temper, zmid, zint, &
                          ...
                          esfact, vmr, invariants, ncol, lchnk, pbuf, mmr, mbar, relhum )
!-----------------------------------------------------------------
...
!-----------------------------------------------------------------
    ...
    real(r8), intent(in)    :: mbar(ncol,pver)              ! Molar mass of air (g/mol) refer to mo_gas_phase
...
!-----------------------------------------------------------------
    ...
    real(r8) :: coa
    real(r8) :: soatot(ncol,pver)
    real(r8) :: clw(ncol,pver)                      
    real(r8) :: KH_Hg2(ncol,pver)                   
    real(r8) :: F_Hg2(ncol,pver)  
    real(r8) :: oasum(ncol,pver,1)
...
!------------------------------------------------------------------------------------------------------------
    ...
    if( jhg2_ndx > 0 .and. jno2_ndx > 0 .and. pom1_ndx > 0 .and. soa11_ndx > 0) then
       do k=1,pver
         do i=1,ncol
           clw(i,k) = lwc(i,k) * invariants(i,k,indexm) * mbar(i,k) / avogadro
           if( clouds(i,k) /= 0 ) then
              clw(i,k) = clw(i,k) / clouds(i,k)
            else
              clw(i,k) = 0._r8
           end if
           KH_Hg2(i,k) = 1.4e4_r8 * exp(5300._r8 * ( 1._r8/temper(i,k) - 1._r8/298.15_r8 ))
           F_Hg2(i,k) = 1._r8 / (1._r8 + KH_Hg2(i,k) * 8.2e-2_r8 * temper(i,k) * clw(i,k))
           soatot(i,k) = vmr(i,k,soa11_ndx) + vmr(i,k,soa12_ndx) + vmr(i,k,soa21_ndx) + vmr(i,k,soa22_ndx) + vmr(i,k,soa31_ndx)+ &
                         vmr(i,k,soa32_ndx) + vmr(i,k,soa41_ndx) + vmr(i,k,soa42_ndx) + vmr(i,k,soa51_ndx) + vmr(i,k,soa52_ndx)
           coa = (vmr(i,k,pom1_ndx)+vmr(i,k,pom4_ndx)) * invariants(i,k,indexm) * adv_mass(pom1_ndx) / avogadro * 1.e12_r8 + &
                      soatot(i,k) * invariants(i,k,indexm) * adv_mass(soa11_ndx) / avogadro * 1.e12_r8 
           photos(i,k,jhg2_ndx) = 8.e-1_r8 * photos(i,k,jno2_ndx) * coa * (1-F_Hg2(i,k)) * clouds(i,k)
          end do
       end do
    end if

    if( jhgp_ndx > 0 .and. jno2_ndx > 0 .and. pom1_ndx > 0 .and. soa11_ndx > 0) then
       do k=1,pver
          do i=1,ncol
             soatot(i,k) = vmr(i,k,soa11_ndx) + vmr(i,k,soa12_ndx) + vmr(i,k,soa21_ndx) + vmr(i,k,soa22_ndx) + vmr(i,k,soa31_ndx)+ &
                         vmr(i,k,soa32_ndx) + vmr(i,k,soa41_ndx) + vmr(i,k,soa42_ndx) + vmr(i,k,soa51_ndx) + vmr(i,k,soa52_ndx)
             coa = (vmr(i,k,pom1_ndx)+vmr(i,k,pom4_ndx)) * invariants(i,k,indexm) * adv_mass(pom1_ndx) / avogadro * 1.e12_r8 + &
                      soatot(i,k) * invariants(i,k,indexm) * adv_mass(soa11_ndx) / avogadro * 1.e12_r8
             if ( relhum(i,k) > 0.35_r8 ) then
                photos(i,k,jhgp_ndx) = 8.e-1_r8 * coa * photos(i,k,jno2_ndx)
              else 
                photos(i,k,jhgp_ndx) = 0._r8
             end if
          end do
	    end do
    end if
