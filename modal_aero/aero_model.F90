...
contains
  ...
  subroutine aero_model_init( pbuf2d )
    ...
    call addfld ('hgp_ddf', horiz_only,  'A', 'kg/m2/s ', 'HgP dry deposition flux at bottom (grav + turb)')
    call addfld ('hgp_tbf', horiz_only,  'A', 'kg/m2/s',  'HgP turbulent dry deposition flux')
    call addfld ('hgp_gvf', horiz_only,  'A', 'kg/m2/s ', 'HgP gravitational dry deposition flux')
    call addfld ('hgp_dtq', (/ 'lev' /), 'A', 'kg/kg/s ', 'HgP dry deposition')
    call addfld ('hgp_ddv', (/ 'lev' /), 'A', 'm/s',      'HgP dry deposition velocity') 
    ...
    call addfld ('hgp_sfwet', horiz_only,  'A','kg/m2/s ','HgP Wet deposition flux at surface')
    call addfld ('hgp_wet',(/ 'lev' /), 'A','kg/kg/s ','HgP wet deposition tendency')
  ...
  ! local vars
    ...
    integer :: i
    !-----------------------------------------------------------------------
    integer  :: HgP_id                  ! add hgp index 
    real(r8) :: hgp_sflx(pcols)         ! add HgP deposition flux
    real(r8) :: dep_trb_hg(pcols)       ! kg/m2/s
    real(r8) :: dep_grv_hg(pcols)       ! kg/m2/s (total of grav and trb)
    real(r8) :: pvmzaer_hg(pcols,pverp) ! sedimentation velocity in Pa
    !-------------------------------------------------------------------------
    ...
    call cnst_get_ind( 'HgP', HgP_id, abort=.false. )
...
!         if (lphase == 1) then
          if ((lphase == 1) .and. (lspec <= nspec_amode(m))) then
             ...
             !      calculate the tendencies and sfc fluxes from the above velocities
              ...
              pvmzaer_hg(:ncol,1)=0._r8
              pvmzaer_hg(:ncol,2:pverp) = vlc_dry(:ncol,:,2)

              call outfld('hgp_ddv', pvmzaer_hg(:,2:pverp), pcols, lchnk )

              pvmzaer_hg(:ncol,2:pverp) = pvmzaer_hg(:ncol,2:pverp) * rho(:ncol,:)*gravit

                call dust_sediment_tend( &
                     ncol,             dt,       state%pint(:,:), state%pmid, state%pdel, state%t , &
                     state%q(:,:,HgP_id),  pvmzaer_hg,  ptend%q(:,:,HgP_id), hgp_sflx  )
              !-------------------------------------------------------------------------!
             ...
             do i=1,ncol
                if (vlc_dry(i,pver,jvlc) /= 0._r8) then
                   dep_trb_hg(i)=hgp_sflx(i)*vlc_trb(i,2)/vlc_dry(i,pver,2)
                   dep_grv_hg(i)=hgp_sflx(i)*vlc_grv(i,pver,2)/vlc_dry(i,pver,2)
                end if
             enddo

             call outfld( 'hgp_ddf', hgp_sflx, pcols, lchnk)
             call outfld( 'hgp_tbf', dep_trb_hg, pcols, lchnk )
             call outfld( 'hgp_gvf', dep_grv_hg, pcols, lchnk )
             call outfld( 'hgp_dtq', ptend%q(:,:,HgP_id), pcols, lchnk)
  ...
  subroutine aero_model_wetdep( state, dt, dlf, cam_out, ptend, pbuf)
    ...
    real(r8) :: dqdt_tmp(pcols,pver) ! temporary array to hold tendency for 1 species
    real(r8) :: dqdt_tmp_hgp(pcols,pver)
       ...
       do lphase = strt_loop,end_loop, stride_loop ! loop over interstitial (1) and cloud-borne (2) forms
          ...
          do lspec = 0, nspec_amode(m)+1 ! loop over number + chem constituents + water
             ...
             if ((lphase == 1) .and. (lspec <= nspec_amode(m))) then
                ...
                q_tmp_hgp(1:ncol,:) = state%q(1:ncol,:,HgP_id) + ptend%q(1:ncol,:,HgP_id)*dt
                if(convproc_do_aer) then
                ...
                if ( (m == 1) .and. (lspec == 1) ) then
                 call wetdepa_v2( state%pmid, state%q(:,:,1), state%pdel, &
                      dep_inputs%cldt, dep_inputs%cldcu, dep_inputs%cmfdqr, &
                      dep_inputs%evapc, dep_inputs%conicw, dep_inputs%prain, dep_inputs%qme, &
                      dep_inputs%evapr, dep_inputs%totcond, q_tmp_hgp, dt, &
                      dqdt_tmp_hgp, iscavt, dep_inputs%cldvcu, dep_inputs%cldvst, &
                      dlf, fracis(:,:,161), sol_factb, ncol, &
                      scavcoefnv(:,:,2), &
                      is_strat_cloudborne=.false.,  &
                      qqcw=qqcw_in(:,:),  &
                      f_act_conv=f_act_conv, &
                ...
                if(convproc_do_aer) then
                   ...
                   ptend%q(1:ncol,:,HgP_id) = ptend%q(1:ncol,:,HgP_id) + dqdt_tmp_hgp(1:ncol,:)

                call outfld('hgp_wet', dqdt_tmp_hgp(:,:), pcols, lchnk)   
                ...
                do k=1,pver
                   do i=1,ncol
                      hgp_sflx(i)=hgp_sflx(i)+dqdt_tmp_hgp(i,k)*state%pdel(i,k)/gravit
                   enddo
                enddo
                call outfld('hgp_sfwet', hgp_sflx, pcols, lchnk)
